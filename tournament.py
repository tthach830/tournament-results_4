...
def playerStandings():
    ...
    conn = psycopg2.connect("dbname='tournament'")
    cur = conn.cursor()
    cur.execute("SELECT PlayerID, PlayerFullname, TotalWins,\
    TotalMatches FROM Standings ORDER BY TotalWins DESC")
    return cur.fetchall()
    conn.commit()
    cur.close()
    conn.close()
...
def swissPairings():
    ...
    conn = psycopg2.connect("dbname='tournament'")
    cur = conn.cursor()
    cur.execute("SELECT PlayerID,PlayerFullname,TotalWins,TotalMatches \
    FROM standings ORDER BY TotalWins DESC")
