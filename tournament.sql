...
CREATE VIEW TotalMatches AS
    select players.PlayerFullName, players.playerID, count(matches.WinnerID) as TotalMatches
    from players
    left join matches
...
CREATE VIEW TotalWins AS
    select Players.PlayerID, Players.PlayerFullName, count(matches.WinnerID) as TotalWins
    from Players
    left join matches
...
select TotalWins.playerID,TotalWins.PlayerFullName,TotalWins.TotalWins,TotalMatches.TotalMatches
from TotalWins
...
-- select * from players; select * from matches; select * from TotalMatches;select * from TotalWins;
